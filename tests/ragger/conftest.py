...
def create_backend(backend: bool, exchange: Application, raises: bool = True):
    instanciated_errors = (partial_error() for partial_error in ERRORS)
    if backend.lower() == "ledgercomm":
        return LedgerCommBackend(exchange.firmware, interface="hid", raises=raises, errors=instanciated_errors)
    elif backend.lower() == "ledgerwallet":
        return LedgerWalletBackend(exchange.firmware, raises=raises, errors=instanciated_errors)
    elif backend.lower() == "speculos":
        args, kwargs = prepare_speculos_args(exchange)
        return SpeculosBackend(*args, exchange.firmware, **kwargs, raises=raises, errors=instanciated_errors)
    else:
